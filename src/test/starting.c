...
event init (t = 0)
{
  ...
  if (!restore ("restart")) {
    ...
    foreach()
      u.x[] = u0*ibm[]; // fixme: with 1 this results in sub-optimal adaptation
  }
  else { // restart
...
ThetaOmega * theta_omega()
{
  ...
  foreach (serial)
    if (ibm[] > 0. && ibm[] < 1.) {
      ...
      ThetaOmega t;
      t.omega = ibm_vorticity (point, u, b, n);
      t.theta = atan2(y, x);
...
void display_omega (int width, int height)
{
  ...
  draw_vof ("ibm", filled = -1, fc = {1,1,1});
  draw_vof ("ibm", lw = 6);
  ...
  foreach (serial)
    if (ibm[] > 0. && ibm[] < 1.) {
      ...
      fprintf (fp, "%g %g %g %g %g %g\n",
	       ...
	       y, // 2
	       atan2(y, x), // 3
	       extrapolate_scalar (point, ibm, b, n, p), // 4
	       area*Delta, // 5
	       ibm_vorticity (point, u, b, n)); // 6
    }
}
...
event adapt (i++) {
  ...
  adapt_wavelet ({ibmsf,u}, (double[]){1e-15,cmax,cmax}, maxlevel, 5);
}
