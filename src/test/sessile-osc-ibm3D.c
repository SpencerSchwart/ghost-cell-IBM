...
event logfile (i++; t <= t_end)
{
  ...
  position (ch, pos, {1,0,0});
  foreach()
    if (cr[] < 1e-3) pos[] = nodata;
  double rmax2 = statsf(pos).max;

  double verr = i == 0? 0: (v1 - v0)/v0 * 100;

  fprintf(stdout, "%d %g %g %g %g %g %g %d %g %g %g %g %g\n", 
      i, t, theta0, v0, v1, verr, avgitr, g_count, hmax, rmax, rmax2, R, V);
  fprintf(stderr, "%d %g %g %g %g %g %g %d %g %g %g %g %g\n", 
      i, t, theta0, v0, v1, verr, avgitr, g_count, hmax, rmax, rmax2, R, V);
  fflush(stdout);
}
