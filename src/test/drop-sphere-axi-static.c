...
int main() {
  ...
  const double theta0_array[7] = {15,30,60,90,120,150,165};
  for (int i = 0; i < 7; ++i) {
...
event init (t = 0) 
{
  ...
  astats st;
  do {
    solid (ibmFake, ibmfFake, sq(x - ci.x) + sq(y - ci.y) - sq(D/2));
    fraction (fFake, - (sq(x - di.x) + sq(y - di.y) - sq(D/2)));
  ...
  fraction (f, - (sq(x - di.x) + sq(y - di.y) - sq(D/2)));
  fraction (ch, - (sq(x - di.x) + sq(y - di.y) - sq(D/2)));

}

event logfile (i++; t <= tend)
{
  if (i == 0) v0 = real_volume(f);
  double v1 = i == 0? v0: 0;
...
#if 0
event movie (t += 0.1; t <= tend && theta0 == 15)
{
  char name[80];
