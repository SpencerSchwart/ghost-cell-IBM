                                  ...
                                  void * data)
{
    ...
    foreach_dimension() {
        ...
        foreach (reduction(max:maxres), nowarning) {
            ...
            if (ibm[] <= 0.5)
                res.x[] = 0;
            //fprintf(stderr, "res.x[]=%g %g %g %g %g %g\n", res.x[], a, g.x[], g.x[1], g.y[], g.y[0,1]);
                   ...
                   int nrelax = 4, scalar * res = NULL)
{
    ...
    scalar rho2[];
    foreach() {
        rho2[] = rho[]*cm[]/(ibm[] + SEPS);
        foreach_dimension() {
            r.x[] = rho2[]* u.x[];
        }
    }

    restriction ({mu, rho2, ibmCells});
