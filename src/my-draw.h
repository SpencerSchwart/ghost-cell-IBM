...
macro draw_lines (bview * view, float color[3], float lw, float _alpha = 1.)
{
  {
    glMatrixMode (GL_PROJECTION);
    glPushMatrix();
    glTranslatef (0., 0., view->lc*view->fov/24.);
    glColor4f (color[0], color[1], color[2], _alpha);
...
#define color_facet()							\
  if (color && (!linear || col.i < 0)) {				\
    Color b = colormap_color (cmap, col.i < 0 ?				\
			      ...
			      min, max);				\
    glColor4f (b.r/255., b.g/255., b.b/255., _alpha);				\
  }
...
macro colorized (float fc[3], bool constant_color,
		 double cmap[NCMAP][3], bool use_texture, float _alpha)
{
  ...
  if (use_texture) {
    GLfloat texture[4*256];
    for (int i = 0; i < 256; i++) {
      Color j = colormap_color (cmap, i/255., 0, 1);
      texture[3*i] = j.r/255.;
      texture[3*i + 1] = j.g/255.;
      texture[3*i + 2] = j.b/255.;
      texture[4*i + 3] = _alpha;
    }
    glTexImage1D (GL_TEXTURE_1D, 0, GL_RGBA, 256,0, GL_RGBA, GL_FLOAT, texture);
  ...
  if (constant_color)
    glColor4f (fc[0], fc[1], fc[2], _alpha);
...
* *lw*: the line width.
* *alpha*: the opacity (1 = opaque, 0 = transparent).
*/
	       ...
	       COLORBAR_PARAMS)
{
  ...
  bview * view = draw();
#if dimension == 2
  bool filled_blend = false;
  if (filled && _alpha < 1.0) {
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    filled_blend = true;
  }
