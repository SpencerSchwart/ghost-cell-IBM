...
#if AXI
# define lambda ((coord){1., 1. + dt/(rho[]+SEPS)*(mu.x[] + mu.x[1] + \
					    mu.y[] + mu.y[0,1])/2./sq(y), 0})
#elif SPHERISYM
# define lambda ((coord){1. + 2.*dt/(rho[]+SEPS)*(mu.x[] + mu.x[1])/sq(x), 0})
#else // !AXI && !SPHERISYM
...
static void relax_viscosity (scalar * a, scalar * b, int l, void * data)
{
...
#endif
  {
    ...
    foreach_dimension() {
      ...
      (SEPS + lambda.x*sq(Delta) + dt/(rho[]+SEPS)*(2.*mu.x[1] + 2.*mu.x[]
#if dimension > 1
				      + mu.y[0,1] + mu.y[]
#endif
#if dimension > 2
				  ...
				  void * data)
{
  ...
  foreach_dimension() {
    ...
    \nabla \cdot (\mu \nabla^T \mathbf{u}) = \nabla^2 (\mu \mathbf{u})
    $$
    which yields the laplacian operator acting on u, avoiding cells along the 
