...
static void vertex_buffer_glColor4f (float r, float g, float b, float _alpha)
{
  if (VertexBuffer.color) {
    struct { float x, y, z, alpha; } color = {r, g, b, _alpha};
    array_append (VertexBuffer.color, &color, 4*sizeof(float));
  }
  else
    glColor4f (r, g, b, _alpha);
}

#define glColor4f vertex_buffer_glColor4f
