/**
A 2D sessile droplet spreading on a flat, inclined plate until it reaches an
...
event end (t = end)
{
  ...
  fprintf (fp2, "%d %g %.5g %.3g %.4g %g %g %g\n", N, theta0, R/sqrt(V/pi), s.stddev,
	   equivalent_contact_angle (R, V)*180./pi, v0, V, R);
  fclose(fp2);
}
...
event adapt (i++)
{
  ...
  adapt_wavelet ({f1, u}, (double[]){1e-3, 1e-3, 1e-3},
