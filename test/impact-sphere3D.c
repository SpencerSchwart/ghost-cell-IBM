...
event dump (t += tout)
{
  scalar * dumplist = {u,p,f,cs,ch,extra,contact_angle};
  char name[80];
...
event movie (t += tout) 
{
  ...
  char name[80];
  sprintf(name, "imgs/%g-back-%.5g.png", theta0, t);
  ...
  mirror (n = {0,-1,0}) {
    ...
    draw_vof("cs", "fs", color="extra", min=-1, max=1);
  }
  save(name);

  sprintf(name, "imgs/%g-crfront-%.5g.png", theta0, t);
  ...
  sprintf(name, "imgs/%g-crfrontu-%.5g.png", theta0, t);
  view(camera="front", fov = 6, tx = -0.025, width = 1000, height = 1000, bg={1,1,1});
  clear();
  draw_vof ("ch", lw = 2, color="u.x", min=-1, max=1, linear=true);
  draw_vof("cs", "fs", fc={0.796875, 0.796875, 0.796875});
  ...
  sprintf(name, "imgs/%g-crisou-%.5g.png", theta0, t);
